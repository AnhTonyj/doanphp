<?php
function uploadFile($fileUpload, $Ten_sp)
{
    $target_dir = "../public/images/";
    $target_file = $target_dir . basename($fileUpload["Ten_sp"]);
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($target_file, PATHINFO_EXTENSION)); // Kiểu file
    $target_file_destination = $target_dir . $Ten_sp; // Tên file đặt lại theo mã
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($fileUpload["tmp_name"], $target_file_destination)) { // $target_file
            echo "The file " . basename($fileUpload["Ten_sp"]) . " has been uploaded.";
        } else {
